...
def count_fingers(hand_landmarks, handedness_label):
    ...
    for i, (tip, pip_) in enumerate([(INDEX_TIP, INDEX_PIP),
                                      (MIDDLE_TIP, MIDDLE_PIP),
                                      (RING_TIP, RING_PIP),
                                      (PINKY_TIP, PINKY_PIP)]):
        if lm[tip].y < lm[pip_].y:
            extended_fingers.append(finger_names[i])
            count += 1

    print(f"Extended fingers: {', '.join(extended_fingers) if extended_fingers else 'None'}")
